...
Copyright © 2026 Software Tree, LLC. All rights reserved.
...
The example uses the **MySQL Sakila sample database** (a MySQL reference database). To demonstrate how to reverse-engineer a subset of the relational schema into a JSON object data model, three tables are specified in the template: `film`, `film_actor`, and `film_category`. The tool inspects these tables, detects their columns, types, nullable flags, and relationships, and generates everything needed to start using JDX ORM with JSON-based objects against those tables.
...
> **Note:** The `.java` source files and `.revjdx` mapping file are **outputs** of the reverse engineering tool, not hand-authored inputs. They are regenerated each time `JDXReverseEngineer` is run.
...
Both scripts automatically discover all `.java` files in the project directory, write them to `sources.txt`, then compile to `bin/`.
They includes necessary JDX and external libraries in the classpath.
...
- View the generated model classes (e.g., `Film`, `Film_Actor`, and `Film_Category`)
- Test database queries and operations on the model classes
...
- Make attribute names more meaningful compared to the original column names. For example, to map a cryptic table column name 'cid' to a semantically meaningful attribute name 'customer_id': 
```
...
- Update `setEnvironment.bat` / `setEnvironment.sh` if a different JDBC driver is needed.
...
## Notes on `sources.txt`

The `sources.txt` file in this directory contains absolute paths from a developer's machine and should not be used directly. The `compile.bat` and `compile.sh` scripts regenerate it automatically by discovering `.java` files in the current directory before each compilation run.
...
**JDX Version:** 05.08 (as indicated in reverse-engineered files)